...
class ClusterFile:
    ...
    def read_cluster_file(handle):
        ...
        if version != 3:
            raise Exception("Cluster file version " + str(version) + " not supported")
...
class ClusterRecord:
    ...
    def read_record(handle, version):
        ...
        (aa_r_dev, ab_r_dev, bb_r_dev) = ClusterFile.read_array(handle, 3, read_float)
        (aa_r_mean, ab_r_mean, bb_r_mean) = ClusterFile.read_array(handle, 3, read_float)
        (aa_theta_dev, ab_theta_dev, bb_theta_dev) = ClusterFile.read_array(handle, 3, read_float)
        (aa_theta_mean, ab_theta_mean, bb_theta_mean) = ClusterFile.read_array(handle, 3, read_float)
        ...
        aa_cluster_stats = ClusterStats(aa_theta_mean, aa_theta_dev, aa_r_mean, aa_r_dev, aa_n)
        ab_cluster_stats = ClusterStats(ab_theta_mean, ab_theta_dev, ab_r_mean, ab_r_dev, ab_n)
        bb_cluster_stats = ClusterStats(bb_theta_mean, bb_theta_dev, bb_r_mean, bb_r_dev, bb_n)
...
# Need cluster file with version 8 or 9 to test ClusterFile and LocusAggregate (from locus_check)
egt_path = '/home/arleg/repos/bead_array_files/data/BovineSNP50_v3_A1_ClusterFile.egt'

with open(egt_path, mode='rb') as src:
    egt = ClusterFile.read_cluster_file(src)
