    ...
    with open(output_filename, "w") as output_handle:
        ...
        output_handle.write(delim.join(
            "Row,Locus_Name,Illumicode_Name,#No_Calls,#Calls,Call_Freq,A/A_Freq,A/B_Freq,B/B_Freq,Minor_Freq,Gentrain_Score,50%_GC_Score,10%_GC_Score,Het_Excess_Freq,ChiTest_P100,Cluster_Sep,AA_T_Mean,AA_T_Std,AB_T_Mean,AB_T_Std,BB_T_Mean,BB_T_Std,AA_R_Mean,AA_R_Std,AB_R_Mean,AB_R_Std,BB_R_Mean,BB_R_Std,Plus/Minus Strand".split(
                ",")) + "\n")
        for (locus, locus_summary) in zip(loci, LocusAggregate.aggregate_samples(samples, loci, summarize_locus,
                                                                                  bpm.normalization_lookups)):
            ...
            for cluster_stats in (
            cluster_record.aa_cluster_stats, cluster_record.ab_cluster_stats, cluster_record.bb_cluster_stats):
...
def main():
    ...
    manifest_file = '/home/arleg/repos/bead_array_files/data/OvineSNP50v2_XT_20006795X356271_A1.bpm'
    # manifest = BeadPoolManifest(manifest_file)
    gtc_directory = '/home/arleg/repos/bead_array_files/data'
    project_name = 'Test'
    cluster_file = '/home/arleg/repos/bead_array_files/data/QCArray_96XT_A1_ClusterFile.egt'

    logger = get_logger()
    driver(gtc_directory, manifest_file, cluster_file,
           output_file, project_name, ",", logger)
