...
nan = float32('nan') # np.nan?

# possible problems
# get_ballele_freqs() - return nans
# get_gender - b str

# useful info
# gtc.get_snp_manifest() - manifest file name
# gtc.get_slide_identifier() - first part of gtc conventional name
...
class GenotypeCalls:
    ...
    def get_gender(self):
        ...
        """
        return self.__get_generic(GenotypeCalls.__ID_GENDER, read_char).decode() # remove decode if read_char used to
        # fetch string everywhere
    ...
    def get_normalization_transforms(self):
        ...
        return self.__get_generic_array(GenotypeCalls.__ID_NORMALIZATION_TRANSFORMS,
                                        NormalizationTransform.read_normalization_transform, 52, 0, None)
...
class NormalizationTransform:
    ...
    @staticmethod
    def rect_to_polar(coordinates):
        """
        Converts normalized x,y intensities to (pseudo) polar co-ordinates (R, theta)
        Args:
            coordinates (float, float): Normalized x,y intensities for probe
        Returns:
...
gtc = GenotypeCalls('/home/arleg/repos/bead_array_files/data/203470490019_R07C01 (2).gtc')
